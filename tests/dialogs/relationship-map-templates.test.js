...
describe("the board template", () => {
	...
	it("compiles and draws a line, its label, its head and both people", () => {
		...
		expect(html).toContain("relmap-line--rose");
	...
	it("draws an empty board without throwing", () => {
		...
		expect(html).toContain("relmap-lines");
	});
	...
	it("draws no second stroke layer over the lines", () => {
		const html = render(boardContext());
		expect(html).not.toContain("relmap-tree");
		expect(html.match(/class="relmap-lines"/g)).toHaveLength(1);
	});
	...
	it("sets the captions in an unstretched layer, in board pixels", () => {
		const html = render(boardContext());
		expect(html).toContain('<svg class="relmap-labels" viewBox="0 0 1200 960"');
		expect(html).not.toMatch(/relmap-labels[^>]*preserveAspectRatio/);
	});
	...
	it("draws every caption in ONE svg root, however many there are", () => {
		...
		expect(html.match(/class="relmap-labels"/g)).toHaveLength(1);
	...
	it("leaves an empty layer for the lit person's captions, last on the board", () => {
		const html = render(boardContext());
		expect(html).toMatch(
			/<svg class="relmap-labels-lit" viewBox="0 0 1200 960"[^>]*aria-hidden="true"[^>]*><\/svg>/);
		...
		expect(html.lastIndexOf("relmap-labels-lit"))
			.toBeGreaterThan(html.lastIndexOf("relmap-node"));
	});
	...
	it("lays an invisible target along every line", () => {
		const html = render(boardContext());
		expect(html).toContain('<path class="relmap-hit" data-relmap-hit="e1"');
	});
	...
	it("marks a stroke the reader broke, and leaves a solid one bare", () => {
		const context = boardContext();
		expect(render(context)).not.toContain("relmap-line--rose is-");
		for (const key of ["dotted", "dashed"]) {
			expect(render({ ...context, edges: [{ ...context.edges[0], broken: key }] }))
				.toContain(`relmap-line--rose is-${key}`);
		}
	});
	...
	it("makes the writing itself the button", () => {
		const html = render(boardContext());
		expect(html).toMatch(/<text class="relmap-label-text" role="button" tabindex="0"/);
		expect(html).toMatch(/aria-label="exes\. Click to change or delete this line\."/);
		expect(html).not.toContain('<button type="button" class="relmap-label"');
	});
...
describe("the window template", () => {
	 ...
	 *
	 * ⚠ IT USED TO BE A TOOLBAR, matched on `.relmap-tools`, and the row is gone: "Add
	 ...
	 */
	const bar = html => html.match(/<div class="relmap-foot-tools">[\s\S]*?<\/div>\s*<\/div>/)[0];
	const context = (over = {}) => ({
		title: "The people of Stillwater", canEdit: true,
		empty: false,
		...
		inkPresets: [{ hex: "#2295bf", show: "#2295bf", name: "Azure" }, { hex: "#0b724f", show: "#0b724f", name: "Deep jade" }],
		inkAcross: 8,
	...
	it("puts the tie bar in the viewport, beside the board and not inside it", () => {
		...
		const view = html.indexOf('class="relmap-view"');
		expect(html.indexOf("relmap-tiebar")).toBeGreaterThan(view);
		expect(html.indexOf("relmap-tiebar")).toBeGreaterThan(html.indexOf("<board/>"));
	});
	...
	it("renders the bar hidden rather than leaving it out", () => {
		expect(render(context())).toMatch(/class="relmap-tiebar"[^>]*hidden/);
	});
	...
	it("opens the bar with the writing field", () => {
		const html = render(context());
		const bar = html.slice(html.indexOf("relmap-tiebar"));
	...
	it("lays the eight out as named swatches", () => {
		const html = render(context());
		expect(html).toMatch(/<div class="relmap-inkpop"[^>]*hidden/);
		for (const [key, name] of [["rose", "Rose"], ["slate", "Slate"]]) {
			...
			expect(html).toContain(`relmap-inkpop-swatch relmap-ink--${key}`);
	...
	it("puts the stroke on the bar as a drawn rule, and the three under it", () => {
		...
		expect(html).toMatch(/data-relmap-tie-mark="dash"[\s\S]*?relmap-tiebar-rule--solid/);
		expect(html).toMatch(/<div class="relmap-dashpop"[^>]*hidden/);
		for (const [key, name] of [["solid", "Solid line"], ["dashed", "Dashed line"], ["dotted", "Dotted line"]]) {
			...
			expect(html).toContain(`relmap-tiebar-rule--${key}`);
	...
	it("puts the size on the bar as a number, and the steps under it", () => {
		...
		expect(html).toMatch(/<div class="relmap-sizepop"[^>]*hidden/);
	...
	it("prints no word on the face of a swatch, and keeps every name on it", () => {
		const html = render(context());
		expect(html).not.toContain("relmap-inkpop-word");
	...
	it("lays the presets out under the eight, each named and carrying its own colour", () => {
		const html = render(context());
		expect(html).toContain("relmap-inkpop-grid--more");
	...
	it("keeps the eight and the board's own colours in one group", () => {
		const html = render(context());
		const pop = html.slice(html.indexOf("relmap-inkpop"));
	...
	it("offers rubbing the line out, at the end of the bar and named", () => {
		...
		expect(bar).toContain("relmap-tiebar-rub");
	...
	// is looking at and nothing in the document. The player who may only read a crowded board
	...
	// ⚠ NEVER HIDDEN. Everything in the footer is about the board in front of the reader, and there
	// is always one, so there is no condition to render it behind.
	it("always offers the footer", () => {
		const foot = html => html.match(/<div class="relmap-foot"[^>]*>/)[0];
	...
	it("explains itself from the whole label", () => {
		const html = render(context());
		expect(html).toMatch(/<label class="relmap-foot-check" data-tooltip="why anybody/);
	});
	...
	it("carries no toolbar row above the board", () => {
		const html = render(context());
		expect(html).not.toContain("relmap-bar");
		expect(html).not.toContain("relmap-tools");
	});
	...
	it("puts the tools after the captions box, on the same footer row", () => {
		const html = render(context());
		const foot = html.slice(html.indexOf('<div class="relmap-foot">'));
		expect(foot.indexOf("relmap-foot-tools"))
	...
	// dropped when the filled button took the end of the row (see `.relmap-history`).
	...
	it("wears the class that paints Add someone as the primary", () => {
		expect(bar(render(context())))
			.toMatch(/class="relmap-tool relmap-add"/);
	});
	...
	it("carries a live region, and it is not inside the board", () => {
		const html = render(context());
		const live = html.indexOf("relmap-live");
	...
	it("always renders the empty panel, and hides it while the map has people on it", () => {
		expect(render(context({ empty: true }))).toContain("relmap-empty");
		expect(render(context({ empty: false }))).toContain("relmap-empty");
	});

	it("marks the panel hidden when the map is populated, and only then", () => {
		const panel = html => html.match(/<div class="relmap-empty"[^>]*>/)[0];
	...
	// wears `relmap-empty-cast` too and is gated on `canEdit` in the same way, so an
	...
	it("carries the empty panel's button on a populated board, ready for it to empty", () => {
		const panel = html => html.match(/<div class="relmap-empty"[\s\S]*?<\/div>/)[0];
	...
	it("offers Add someone on an empty board, not a way to add everybody at once", () => {
		const panel = render(context({ empty: true })).match(/<div class="relmap-empty"[\s\S]*?<\/div>/)[0];
	...
	it("carries one panel over the board and no second one", () => {
		...
		expect(html).not.toContain("relmap-nokin");
		expect(html).not.toContain("relmap-aside");
		expect(html.match(/class="relmap-empty"/g)).toHaveLength(1);
	});
});
...
describe("the page strip", () => {
	const render = compile(WINDOW);
	const context = (over = {}) => ({
		title: "The people of Stillwater", canEdit: true, showBoardTools: true,
	...
	it("is absent entirely when the window says not to show it", () => {
		...
		expect(html).not.toContain("relmap-pages");
	...
	it("makes the board the panel the strip names", () => {
		...
		expect(html).toContain('aria-labelledby="relmap-map1-page-p1"');
	});
	...
	it("stands the new-page plus between the tabs and the other two tools", () => {
		const html = render(context());
		const strip = html.indexOf("relmap-pages-strip");
		const plus = html.indexOf('data-relmap-action="pagenew"');
		const group = html.indexOf("relmap-pages-tools");
	...
	it("keeps that plus out of the strip that scrolls", () => {
		...
		const strip = html.match(
			/<div class="relmap-pages-strip"[\s\S]*?<\/div>/,
		)[0];
	...
	it("leaves rename and delete alone together in the tools group", () => {
		...
		const group = html.match(
			/<div class="relmap-pages-tools">([\s\S]*?)<\/div>/,
		)[1];
...
describe("a portrait's box is the circle and nothing else", () => {
	const css = read("styles/relationship-map.css").replace(/\/\*[\s\S]*?\*\//g, "");
	// EVERY block whose whole selector is this class, joined. `.relmap-name` carries
