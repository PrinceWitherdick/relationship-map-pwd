...
function entryFor(graph, { isOwner = true, id = "map1", pen = null } = {}) {
	...
	return {
		id,
		name: "The people of Stillwater",
		isOwner,
		updates: [],
		getFlag: (scope, key) => (scope === "relationship-map-pwd" ? flags[key] ?? null : null),
...
function windowFor(graph = TWO_PEOPLE, {
	...
	const foot = el();
	const root = el();
	root.children[".relmap-board"] = board;
	root.children[".relmap-live"] = live;
	root.children[".relmap-empty"] = empty;
	...
	empty.children[".relmap-empty-lead"] = emptyLead;
	empty.children[".relmap-empty-hint"] = emptyHint;
	empty.children[".relmap-empty-cast"] = emptyCta;
	root.children[".relmap-foot"] = foot;
	...
	const strip = el();
	strip.attrs = {};
	root.children[".relmap-pages-strip"] = strip;
	const view = el();
	view.attrs = {};
	view.setAttribute = (key, value) => { view.attrs[key] = value; };
	root.children[".relmap-view"] = view;
	const dropTool = el();
...
describe("the update hook", () => {
	...
	it("ignores a write to a different journal", () => {
		...
		listener({ id: "someone-else" }, { flags: { "relationship-map-pwd": { relationshipMap: {} } } });
	...
	it("ignores a write to this journal that is not the map", () => {
		...
		listener({ id: "map1" }, { flags: { "relationship-map-pwd": { somethingElse: 1 } } });
		expect(app.sync).not.toHaveBeenCalled();
	});

	// ⚠ The package id is hyphenated, so a DOTTED read of it (`changed.flags.relationship-map-pwd`) parses
	...
	it("does not throw on the hyphenated flag scope, in the shape Foundry sends it", () => {
		...
		expect(() => listener({ id: "map1" }, {
			flags: { "relationship-map-pwd": { relationshipMap: { nodes: { elena: { x: 4 } } } } },
		})).not.toThrow();
	});
	...
	it("repaints on a write to the map", async () => {
		const { app } = windowFor();
		const listener = hookFor(app);
		listener({ id: "map1" }, { flags: { "relationship-map-pwd": { relationshipMap: { nodes: {} } } } });
	...
	it("repaints on a DELETION, whose key arrives prefixed", async () => {
		const { app } = windowFor();
		const listener = hookFor(app);
		listener({ id: "map1" }, { flags: { "relationship-map-pwd": { "-=relationshipMap": null } } });
		await new Promise(r => setTimeout(r, 80));
		expect(app.sync).toHaveBeenCalled();
	});
...
describe("cutting each gap to the caption that actually painted", () => {
	...
	function bare() {
		...
		made.board.children[".relmap-labels"] = layer;
		made.board.all = {
			"[data-relmap-line]": [line],
			"[data-relmap-edge]": [],
			"[data-relmap-words]": [],
	...
	it("mints a caption for a line that has never carried one, so the first letter shows at once", () => {
		...
		expect(words.attrs.class).toBe("relmap-label-text");
		expect(words.textContent).toBe("wed");
	});
...
describe("what a portrait says when it is rested on", () => {
	...
	it("names a deleted actor in words and stops offering a sheet", () => {
		expect(tooltips().ghost)
			.toContain("Tobin (deleted actor). Their actor is no longer in this world.");
...
describe("opening a map", () => {
	...
	it("gives each map its own window id", () => {
		...
		expect(opened).toEqual(["relationship-map-pwd-map1", "relationship-map-pwd-map2"]);
...
//
// A crowded board carries a hundred captioned lines, and at the zoom that fits the whole of it
...
describe("lighting one person's web", () => {
	...
	function caption(id, { lit = false } = {}) {
		...
		return node("g", {
			classes: lit ? ["relmap-label", "is-lit"] : ["relmap-label"],
	...
	function boardWithCaptions() {
		...
		const lift = node("svg", { classes: ["relmap-labels-lit"] });
		root.children[".relmap-labels-lit"] = lift;
		root.all[".relmap-labels .relmap-label"] = captions;
	...
	it("draws a COPY of each lit caption into the layer over the top", () => {
		...
		expect(lift.kids.every(g => g.classList.contains("relmap-label"))).toBe(true);
	});
	...
	it("never moves a caption out of the layer it was painted in", () => {
		...
		expect(captions[0].querySelectorAll(".relmap-label-text")).toHaveLength(1);
	});
	...
	it("strips everything that would make the copy a second button", () => {
		...
		const [words] = copy.querySelectorAll(".relmap-label-text");
...
describe("the panel a repaint puts up", () => {
	...
	it("keeps the empty board's headline through a repaint", () => {
		...
		expect(made.emptyLead.textContent).toBe(TABLE.localize("RELMAP.emptyLead"));
...
describe("what happens to the document under the window", () => {
	...
	// \u26a0 NOTHING ELSE WOULD NOTICE. This is a RelmapDialog rather than a DocumentSheet, so it
...
describe("picking up the pen a reader drew with", () => {
	...
	const PEN_PREFIX = "flags.relationship-map-pwd.relationshipMapPen";
...
describe("rubbing a line out from the bar", () => {
	...
	it("takes the line off the map without a question", async () => {
		...
		expect(Object.keys(entry.updates[0]))
			.toEqual(["flags.relationship-map-pwd.relationshipMap.edges.-=link1"]);
	});
...
function pageFor(name, graph, { id, sort, parent, ownership = null }) {
	const doc = {
		...
		getFlag: (scope, key) =>
			(scope === "relationship-map-pwd" && key === "relationshipMap" ? graph : null),
...
function pagedEntry(boards, { isOwner = true } = {}) {
	...
	const entry = {
		...
		getFlag: (scope, key) =>
			(scope === "relationship-map-pwd" && key === "relationshipMap" ? { version: 2 } : null),
		...
		createEmbeddedDocuments(type, rows) {
			const made = rows.map((row, i) => pageFor(
				row.name, row.flags?.["relationship-map-pwd"]?.relationshipMap ?? null,
...
const TWO_BOARDS = () => pagedEntry([
	{ id: "p1", name: "Stillwater", graph: TWO_PEOPLE },
...
describe("the pages of one map", () => {
	it("names every board in the strip, and marks the one that is up", () => {
		...
		expect(tabs).toContain(">Stillwater<");
		expect(tabs).toContain(">Marshford<");
	...
	it("reads the board the reader is looking at, and not another one", () => {
		const entry = TWO_BOARDS();
		const onStillwater = windowFor(null, { entry, pageId: "p1" });
		expect(Object.keys(readGraph(onStillwater.app.boardDoc).nodes)).toEqual(["elena", "stefan"]);
		const onMarshford = windowFor(null, { entry, pageId: "p2" });
		expect(Object.keys(readGraph(onMarshford.app.boardDoc).nodes)).toEqual([]);
	});
	...
	it("writes to that board's own document and to nothing else", async () => {
		...
		const [stillwater, marshford] = entry.pages.contents;
		expect(stillwater.updates).toHaveLength(1);
		...
		expect(Object.keys(stillwater.updates[0])).toEqual([
			"flags.relationship-map-pwd.relationshipMap.nodes.elena.x",
			"flags.relationship-map-pwd.relationshipMap.nodes.elena.y",
		]);
		expect(marshford.updates).toEqual([]);
		expect(entry.updates).toEqual([]);
	});
	...
	it("leaves the strip alone on a repaint that did not change it", () => {
		...
		app._paintPages();
		expect(strip.innerHTML).toContain("Stillwater");
	...
	it("writes the strip again when a board is renamed at the far end of the table", () => {
		...
		expect(strip.innerHTML).not.toContain("Marshford");
	});
	...
	it("gives a map that still has no pages one tab named after the map", () => {
		...
		expect(app._pageTabs()).toContain(">The people of Stillwater<");
...
describe("keeping up with pages being changed elsewhere", () => {
	...
	const GRAPH_WRITE = { flags: { "relationship-map-pwd": { relationshipMap: { nodes: {} } } } };
	...
	it("puts a board added elsewhere into the strip without re-rendering", () => {
		...
		app._wireSync();
		app._paintPages();
		const made = pageFor("The Masons", EMPTY_BOARD, { id: "p3", sort: 300000, parent: entry });
		entry.pages.contents.push(made);
		on.get("createJournalEntryPage")(made);
		expect(strip.innerHTML).toContain("The Masons");
		expect(app.render).not.toHaveBeenCalled();
	});
	...
	it("moves this reader off a board somebody else has rubbed out, and re-renders", () => {
		...
		expect(app.mapPage.name).toBe("Marshford");
		expect(app.render).toHaveBeenCalled();
	});
...
describe("adding a board from the strip", () => {
	...
	it("makes the board and takes the reader straight to it", async () => {
		...
		expect(made.name).toBe("The Masons");
	...
	it("says the board has been added rather than merely that it is showing", async () => {
		...
		app._askPageName = vi.fn().mockResolvedValue("The Masons");
		await app._addPage();
		expect(app._sayOnRender).toContain("The Masons");
		expect(app._sayOnRender).not.toBe(
			TABLE.format("RELMAP.pages.now", { name: "The Masons" }));
	});
...
function stripWindow() {
	const entry = pagedEntry([
		...
		{ id: "p2", name: "Marshford" },
		{ id: "p3", name: "The Masons" },
	]);
...
describe("putting the boards in an order", () => {
	...
	it("does not offer it to a reader who may only look", () => {
		const entry = pagedEntry([{ id: "p1", name: "Stillwater" }, { id: "p2", name: "Marshford" }],
			{ isOwner: false });
		const { app } = windowFor(null, { entry, pageId: "p1" });
	...
	it("puts a tab in front of the one whose front half it was let go over", async () => {
		...
		expect(boardOrder(entry)).toEqual(["The Masons", "Stillwater", "Marshford"]);
	});
	...
	it("puts it behind when the pointer is on the back half", async () => {
		...
		expect(boardOrder(entry)).toEqual(["Stillwater", "The Masons", "Marshford"]);
	});
	...
	it("puts it on the far end when it is let go over the back half of the last tab", async () => {
		...
		expect(boardOrder(entry)).toEqual(["Marshford", "The Masons", "Stillwater"]);
	});
	...
	it("takes no notice of a drag that did not start on the strip", async () => {
		...
		expect(boardOrder(entry)).toEqual(["Stillwater", "Marshford", "The Masons"]);
	});
	...
	it("moves the tab one place along on Ctrl and an arrow", async () => {
		...
		expect(boardOrder(right.entry)).toEqual(["Marshford", "Stillwater", "The Masons"]);
		...
		expect(boardOrder(left.entry)).toEqual(["Stillwater", "The Masons", "Marshford"]);
	});
	...
	it("says which place the board came to rest in", async () => {
		...
		expect(live.textContent).toBe(TABLE.format("RELMAP.pages.moved", {
			name: "Stillwater", index: 3, count: 3,
		}));
	});
	...
	describe("when the write is refused", () => {
		...
		it("says so instead of throwing", async () => {
			...
			expect(globalThis.ui.notifications.warn)
				.toHaveBeenCalledWith(TABLE.localize("RELMAP.pages.moveFailed"));
			...
			expect(live.textContent).toBe(TABLE.localize("RELMAP.pages.moveFailed"));
			expect(boardOrder(entry)).toEqual(["Stillwater", "Marshford", "The Masons"]);
		});
...
const FLAG = "flags.relationship-map-pwd.relationshipMap";
...
function livingMap(boards) {
	...
	boards.forEach((board, i) => {
		const page = {
			...
			getFlag: (scope, key) =>
				(scope === "relationship-map-pwd" && key === "relationshipMap" ? page.flag : null),
			update(patch) {
...
const ONE_LIVING_BOARD = () => livingMap([{ id: "p1", name: "Stillwater", graph: TWO_PEOPLE }]);
...
describe("taking a change back", () => {
	...
	it("gives each board of a map its own history", async () => {
		const { entry } = livingMap([
			{ id: "p1", name: "Stillwater", graph: TWO_PEOPLE },
			{ id: "p2", name: "Marshford", graph: TWO_PEOPLE },
		]);
		const { app } = windowFor(null, { entry, pageId: "p1" });
		await app._moveNode("elena", { x: 80, y: 90 });
		expect(app._history.canUndo).toBe(true);
...
describe("hiding a board from the players", () => {
	...
	const MIXED = () => pagedEntry([
		{ id: "p1", name: "Stillwater", graph: TWO_PEOPLE },
		{ id: "p2", name: "Marshford", graph: EMPTY_BOARD, hidden: true },
	]);
	...
	it("tells that player what they are looking at instead", () => {
		...
		expect(said.emptyLead).toBe("RELMAP.unsharedLead");
		expect(said.emptyHint).toBe("RELMAP.unsharedHint");
	...
	it("shows an open eye on a shown board and a struck one on a hidden board", () => {
		...
		expect(shown.pageHideHint).toBe("RELMAP.pages.hideHint");
		...
		expect(dark.pageHideHint).toBe("RELMAP.pages.showHint");
	});
	...
	it("hides the board that is up, and says so", async () => {
		...
		expect(live.textContent).toBe("RELMAP.pages.hidden");
	});
	...
	it("shows it again, and hands the right to edit it back with it", async () => {
		...
		expect(live.textContent).toBe("RELMAP.pages.shown");
	});
	...
	it("writes the eye again when a board is hidden under an open window", () => {
		...
		expect(seenTool.dataset.tooltip).toBe("RELMAP.pages.hideHint");
		expect(seenTool.attrs["aria-label"]).toBe("RELMAP.pages.hideHint");
		...
		expect(seenTool.dataset.tooltip).toBe("RELMAP.pages.showHint");
	...
	describe("and showing one again", () => {
		...
		const withNoStrip = made => { delete made.root.children[".relmap-pages-strip"]; return made; };
...
describe("a window shut while it was still drawing", () => {
	...
	it("unwires the hooks its own render registered after it was shut", async () => {
		...
		// RelmapDialog's `close` outright and never put it back, so a spy any further up the
		...
		try {
			...
			// The two members RelmapDialog's own `_render` and `close` reach for, which the
...
describe("how heavily this reader wants the board drawn", () => {
	...
	it("says the new weight out loud", async () => {
		...
		expect(live.textContent).toContain(TABLE.localize("RELMAP.weights.word.name"));
	});
