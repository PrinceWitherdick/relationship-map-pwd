...
//
// WHAT IT MODELS IS A GESTURE: a sequence of pointer events whose meaning is in their ORDER and in
// what the code does between them, such as when it takes the pointer capture and whether a click
// still arrives at the thing pressed. So it has a pointer, a capture, an animation frame and a
// `style` to write to, none of which a fake that only answers what `closest` walks up to needs.
//
...
export function pointerBoard({ nodes = ["n1", "n2"], edges = ["e1"] } = {}) {
	...
	const root = boardEl();
	const view = boardEl({ cls: ["relmap-view"], parent: root });
	const board = boardEl({ cls: ["relmap-board"], parent: view });
	...
	// past the handle to `.relmap-node`, which carries no such attribute, and finds
	...
	for (const id of nodes) {
		const el = boardEl({ cls: ["relmap-node"], dataset: { relmapNode: id }, parent: board });
		el.face = boardEl({ cls: ["relmap-face"], dataset: { relmapOpen: id }, parent: el });
		el.name = boardEl({ cls: ["relmap-name"], parent: el });
		el.handle = boardEl({ cls: ["relmap-handle"], dataset: { relmapHandle: id }, parent: el });
		...
		el.bin = boardEl({ cls: ["relmap-bin"], dataset: { relmapRemove: id }, parent: el });
		portraits[id] = el;
	}
	...
	const labels = boardEl({ cls: ["relmap-labels"], parent: board });
	const captions = {};
	for (const id of edges) {
		const g = boardEl({ cls: ["relmap-label"], dataset: { relmapEdge: id }, parent: labels });
		g.words = boardEl({ cls: ["relmap-label-text"], dataset: { relmapWords: id }, parent: g });
		captions[id] = g;
	}
	...
	const lines = boardEl({ cls: ["relmap-lines"], parent: board });
	const strokes = {};
	for (const id of edges) {
		const painted = boardEl({ cls: ["relmap-line"], dataset: { relmapLine: id }, parent: lines });
		painted.hit = boardEl({ cls: ["relmap-hit"], dataset: { relmapHit: id }, parent: lines });
		strokes[id] = painted;
	}
	...
	const tiebar = boardEl({ cls: ["relmap-tiebar"], parent: view });
	tiebar.hidden = true;
