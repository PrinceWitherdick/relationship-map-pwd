...
import { boardEl, fakeSurface, pointerBoard } from "../fakes/pointer-board.js";
...
describe("a portrait carried while the board is zoomed", () => {
	/** A surface whose scale and pan a test can change mid-gesture, the way the wheel does. */
	function zoomable() {
		const s = { scale: 1, per: 10, off: 0 };
		return Object.assign(s, {
			caughtGlide: false,
			deltaToPercent: vi.fn((dx, dy) => ({ left: dx / s.per, top: dy / s.per })),
			pointToPercent: vi.fn(({ clientX, clientY }) => ({ left: (clientX - s.off) / s.per, top: clientY / s.per })),
		});
	}
