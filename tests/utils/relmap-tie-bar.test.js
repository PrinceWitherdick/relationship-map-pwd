...
function barDom({
	...
	const bar = boardEl({ cls: ["relmap-tiebar"], parent: view });
	bar.hidden = true;
	...
	const mark = boardEl({ cls: [`relmap-ink--${inks[inks.length - 1] ?? "slate"}`],
		dataset: { relmapTieMark: "ink" }, parent: trigger });
	const word = boardEl({ dataset: { relmapTieName: "ink" }, parent: trigger });

	const pop = boardEl({
		cls: ["relmap-inkpop"], dataset: { relmapTiePop: "ink" }, parent: bar,
	});
	pop.hidden = true;
	...
	const dashMark = boardEl({
		cls: ["relmap-tiebar-rule", "relmap-tiebar-rule--solid"],
		dataset: { relmapTieMark: "dash" }, parent: dashTrigger,
	});

	const dashPop = boardEl({
		cls: ["relmap-dashpop"], dataset: { relmapTiePop: "dash" }, parent: bar,
	});
	dashPop.hidden = true;
	...
	const sizePop = boardEl({
		cls: ["relmap-sizepop"], dataset: { relmapTiePop: "size" }, parent: bar,
	});
	sizePop.hidden = true;
...
describe("opening the bar on a line", () => {
	...
	it("shows the colour, the arrow and the stroke the line already has", () => {
		...
		expect(dom.dashMark.classes).toContain("relmap-tiebar-rule--dotted");
		expect(dom.dashMark.classes).not.toContain("relmap-tiebar-rule--solid");
	...
	it("paints the swatch beside it in that colour and no other", () => {
		const { bar } = make(dom);
		bar.open("e1");
		expect(dom.mark.classes).toContain("relmap-ink--rose");
		expect(dom.mark.classes).not.toContain("relmap-ink--slate");
	});
...
describe("the palette", () => {
	...
	it("paints the disc and the word at once, before anything is written", () => {
		...
		bar.open("e1");
		chooseInk(dom, "slate");
		expect(dom.mark.classes).toContain("relmap-ink--slate");
		expect(dom.mark.classes).not.toContain("relmap-ink--rose");
...
describe("the colours the palette offers that nobody named", () => {
	...
	it("draws the line in one at a press, and stores it as the hex it is", () => {
		...
		expect(dom.mark.classes).toContain("relmap-ink--custom");
...
describe("a colour of the reader's own", () => {
	...
	it("opens the picker on the colour the line is already wearing", () => {
		...
		globalThis.getComputedStyle = () => ({
			getPropertyValue: name => (name === "--relmap-ink-rose" ? "hsl(340deg 62% 60%)" : ""),
		});
		try {
			const { bar } = make(dom);
			bar.open("e1");
			...
			expect(dom.picker.value).toBe(resolveInkHex("rose", globalThis.document.documentElement));
	...
	it("puts the picker away again when a named colour is chosen", () => {
		...
		expect(dom.picker.hidden).toBe(true);
		expect(dom.mark.classes).toContain("relmap-ink--slate");
		expect(dom.mark.classes).not.toContain("relmap-ink--custom");
...
describe("the stroke", () => {
	...
	it("draws and names the chosen stroke on the bar without waiting for the write", () => {
		...
		bar.open("e1");
		chooseDash(dom, "dashed");
		expect(dom.dashMark.classes).toContain("relmap-tiebar-rule--dashed");
		expect(dom.dashMark.classes).not.toContain("relmap-tiebar-rule--dotted");
	...
	it("falls back to a whole stroke for an answer it does not offer", () => {
		...
		bar.open("e1");
		expect(dom.dashMark.classes).toContain("relmap-tiebar-rule--solid");
