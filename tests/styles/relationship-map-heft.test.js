...
// and pins its `font-size` in PIXELS; relationship-map.css is unlayered, so it beats core per PROPERTY --
...
//
// It got that far because the browser check written for it mounted the markup under a bare
// `<div class="relmap">` with the module's stylesheet alone, so neither core declaration was in the
// cascade and the shot was clean. The honest check loads foundry2.css first and puts the markup
// inside a real `.app.window-app`, and this file holds the source-level facts it measured, so the
// next hand in here cannot quietly drop them.
...
describe("the readout, which is also the way back to 100%", () => {
	...
	it("names its own width rather than taking core's full-width one", () => {
		const block = rule(".relmap-heft-now");
	...
	it("cannot be shrunk back by the line it overflows", () => {
		expect(rule(".relmap-heft-now")).toMatch(/flex:\s*none/);
	});
	...
	it("takes core's button margin off", () => {
		expect(rule(".relmap-heft-now")).toMatch(/margin:\s*0;/);
	});
});
...
describe("the two arrows", () => {
	...
	it("are a square that does not move with the line", () => {
		const block = rule(".relmap-heft-step");
	...
	it("takes core's button margin off", () => {
		expect(rule(".relmap-heft-step")).toMatch(/margin:\s*0;/);
	});
	...
	it("takes core's icon-to-label margin off the glyph inside it", () => {
		expect(rule(".relmap-heft-step > i")).toMatch(/margin:\s*0;/);
	});
});
...
describe("what size the dials are", () => {
	it("takes its size from the group and not from core's pixels", () => {
		expect(rule(".relmap-heft")).toMatch(/font-size:\s*var\(--relmap-fs-sm\)/);
		expect(rule(".relmap-heft-step")).toMatch(/font-size:\s*inherit/);
		expect(rule(".relmap-heft-now")).toMatch(/font-size:\s*inherit/);
	});

	// The captions box beside it is `--relmap-fs-sm` too. One token, one line, no arithmetic to drift.
	it("is the size the captions box beside it is set in", () => {
		expect(rule(".relmap-foot-check")).toMatch(/font-size:\s*var\(--relmap-fs-sm\)/);
	});
});
...
describe("where the three dials stand", () => {
	...
	it("is not hidden until something is hovered", () => {
		expect(CSS).not.toContain(".relmap-view:hover .relmap-heft");
	...
	it("wraps rather than losing a dial off the end of the line", () => {
		expect(rule(".relmap-heft")).toMatch(/flex-wrap:\s*wrap/);
		expect(rule(".relmap-foot")).toMatch(/flex-wrap:\s*wrap/);
	});
	...
	it("fences each dial with a rule of its own", () => {
		const block = rule(".relmap-heft-dial");
		expect(block, "no rule for the dial").toBeTruthy();
		expect(block).toMatch(/border:\s*1px solid var\(--relmap-rule\)/);
	...
	it("gives the mark more air than the dial's own gap before the minus", () => {
		const gap = Number(rule(".relmap-heft-dial").match(/gap:\s*([\d.]+)em/)[1]);
		const [, right] = rule(".relmap-heft-mark").match(/margin:\s*0 ([\d.]+)em/);
	...
	it("spaces the boxes the way the footer spaces its groups", () => {
		const outer = rule(".relmap-heft").match(/gap:\s*([\d.]+)em/)[1];
		const foot = rule(".relmap-foot").match(/gap:\s*([\d.]+)em/)[1];
	...
	it("is on the footer beside the captions box, and not behind the editing gate", () => {
		const foot = HBS.slice(HBS.indexOf('<div class="relmap-foot">'));
		const check = foot.indexOf("relmap-foot-check");
		const heft = foot.indexOf('<div class="relmap-heft"');
	...
	it("keeps the attribute the click handler dispatches on", () => {
		const dial = HBS.slice(
			HBS.indexOf('<div class="relmap-heft-dial"'),
			HBS.indexOf("{{/each}}", HBS.indexOf('<div class="relmap-heft-dial"')),
		);
