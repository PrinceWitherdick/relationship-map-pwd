...
const ROOT_RULE = declarations(CSS, ".relmap");
...
function directChildClasses() {
	...
	for (const line of WINDOW_TEMPLATE.split("\n")) {
		const m = line.match(/^ {2}<(?:div|p|section|nav)\s+class="(relmap-[a-z-]+)/);
		if (m) found.add(m[1]);
	}
	return [...found];
}
...
describe("the relationship map window's grid", () => {
	...
	it("puts the viewport on the row that takes what is left", () => {
		...
		const viewportRow = Number(gridRow(".relmap-view"));
...
describe("the page strip inside that row", () => {
	...
	it("lets its tabs keep their width", () => {
		expect(declarations(CSS, ".relmap-page")).toMatch(/flex\s*:\s*none/);
	});

	it("scrolls sideways rather than wrapping or pushing the tools off", () => {
		const strip = declarations(CSS, ".relmap-pages-strip");
		...
		expect(declarations(CSS, ".relmap-pages-tools")).toMatch(/flex\s*:\s*none/);
	});
	...
	it("spends the row's spare width after the plus, not before it", () => {
		const plus = declarations(CSS, ".relmap-pages .relmap-page-tool--new");
	...
	it("marks the current tab by more than its colour", () => {
		const current = declarations(CSS, ".relmap-page.is-current");
...
describe("a board the players can see", () => {
	...
	it("leaves the tabs carrying nothing but their name", () => {
		const tab = declarations(CSS, ".relmap-page");
	...
	it("lights the eye on a board the table is looking at, hover included", () => {
		const lit = declarations(
			CSS,
			".relmap-page-tool--seen:not(.is-hidden-board):focus-visible",
		);
		expect(lit).toMatch(/color\s*:/);
		expect(CSS.indexOf(".relmap-page-tool--seen"))
			.toBeGreaterThan(CSS.indexOf(".relmap-page-tool:hover"));
	});
});
