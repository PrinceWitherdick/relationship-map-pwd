...
const PREFIX = `flags.relationship-map-pwd.${RELMAP_PEN_FLAG}`;
...
function makeMap({ pen = null, isOwner = true } = {}) {
	const flags = pen ? { "relationship-map-pwd": { [RELMAP_PEN_FLAG]: pen } } : {};
	return {
		isOwner,
		flags,
		updates: [],
...
describe("the write that records a pen", () => {
	...
	it("sanitises on the way in as well as on the way out", () => {
		expect(penPatch({ ink: "chartreuse" })).toEqual({ [`${PREFIX}.ink`]: RELMAP_INK_DEFAULT });
