...
const mapFlag = { [MODULE_ID]: { relationshipMap: { version: 2 } } };
...
describe("isRelationshipMapEntry", () => {
	it("goes by the flag and not by the name", () => {
		expect(isRelationshipMapEntry(entryDoc("a", "Stillwater", { map: true }))).toBe(true);
		// A prose entry a GM happened to call the same thing is not a map.
		expect(isRelationshipMapEntry(entryDoc("b", "Stillwater"))).toBe(false);
...
describe("isRelationshipMapFolder", () => {
	...
	it("goes when everything in it is a map", () => {
		expect(isRelationshipMapFolder(folder({ contents: [entryDoc("a", "Stillwater", { map: true })] }))).toBe(true);
	});

	it("stays when a GM has filed anything else beside them", () => {
		expect(isRelationshipMapFolder(folder({
			contents: [entryDoc("a", "Stillwater", { map: true }), entryDoc("b", "Notes on the town")],
		}))).toBe(false);
	});
...
describe("hideRelationshipMapRows", () => {
	it("takes the map rows out and leaves every other journal alone", () => {
		const maps = entryDoc("map", "Stillwater", { map: true });
		const lore = entryDoc("lore", "The Gods of Stillwater");
	...
	it("takes the folder with them when nothing else is in it", () => {
		const map = entryDoc("map", "Stillwater", { map: true });
