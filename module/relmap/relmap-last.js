...
//
// "OPEN THE MAPS" ASKS NOBODY. A table has one map it lives on, opens it many times a session, and
// would pick the same row out of a list every time. So opening the maps goes straight to the board
// this client was last looking at; the Maps button on the map window, and its page strip, are how a
// reader goes somewhere else.
//
// PER CLIENT, NEVER PER WORLD. Which board somebody is reading is theirs, exactly like the page they
// are on inside it and the corner they have zoomed into: a GM opening one board must not move where
// a player's button lands. So this is a client-scoped setting, and no document is written for it.
//
// ⚠ NESTED UNDER THE WORLD ID, which a client setting has to do for itself: it lives in browser
// localStorage under `namespace.key` alone, with no world in the path. A flat record would hand this
// world an entry id belonging to a different one. Nothing would misread it (the id simply resolves to
...
/**
 * Record the board this reader is on. Called from every render of the window, so it covers opening a
 * map, switching page inside it, and a window restored across a reload.
 *
 ...
 * portrait dropped, a name rewritten), and a setting write is a localStorage write plus an onChange.
 *
...
export function rememberBoard(entryId, pageId = null) {
	...
	return setSettingQuietly(RELMAP_LAST_SETTING, all,
		"Relationship Map | remembering the open relationship board failed");
}

/**
 * Which map, and which of its boards, an unasked open lands on: `{ entry, pageId }`, or null when the
 * world has no maps at all (the caller offers to make the first one).
 *
 * A REMEMBERED PAGE IS CHECKED BEFORE IT IS USED, and a stale one falls back to that map's first
 * board rather than throwing the whole record away. Pages are deleted under an open window by anyone
 * at the table, so "the map I was on, at a page that is no longer there" is an ordinary thing to hold.
 */
