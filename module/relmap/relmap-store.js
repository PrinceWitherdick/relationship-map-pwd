...
// the window, the drag layer and the tie bar is how one of them ends up doing that.
//
...
// `isSafeId` is the guard, every builder below refuses anything else, and a test holds it there.
...
export function readInk(value) {
	if (RELMAP_INKS.includes(value)) return value;
	return normalizeHex(value) || RELMAP_INK_DEFAULT;
}
 ...
 * it. Every caller that seats people goes through here, so none of them can forget the `?? {}`.
 *
...
export function addEdgePatch(id, {
	...
	return edgePatch(id, { a, b, label, ink, dir, dash, size, seat, note });
}
