...
export function getPen(entry) {
	return readPen(entry?.getFlag?.(MODULE_ID, RELMAP_PEN_FLAG));
}
...
export function rememberPen(entry, fields = {}) {
	...
	const blame = err => console.error("Relationship Map | remembering the relationship map's pen failed", err);
	try {
