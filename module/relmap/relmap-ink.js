...
/**
 * The two tones a board is drawn on in each theme, when there is no document to read them off.
 *
 * ⚠ A COPY OF THE STYLESHEET, and held to it by a test rather than by anybody remembering. These are
 * here because the arithmetic has to work where there is no computed style to ask (a test, or the
 * very first render, before the window has an element), and silently measuring against the wrong
 * paper is worse than measuring against a stale copy that a test fails over.
 */
export const RELMAP_GROUND_FALLBACK = Object.freeze({
	light: Object.freeze({ page: "hsl(30deg 20% 98%)", panel: "hsl(30deg 20% 94%)" }),
	dark: Object.freeze({ page: "hsl(235deg 12% 13%)", panel: "hsl(235deg 11% 17%)" }),
});
...
/**
 * The two tones a board is actually painted on, read off the window.
 *
 * READ RATHER THAN ASSUMED, so this measures against the paper the reader is looking at, which moves
 * between Foundry's light and dark themes and under the high-contrast board.
 *
 * @param {HTMLElement|null} [root]  an element inside the window (the tokens are declared on the
 *        window root and inherited), or null where there is none yet.
 * @param {"light"|"dark"} [scheme]  which theme's own values stand in when nothing can be read.
 * @returns {Array<number[]>}  the grounds, as rgb triples. Never empty: a window that answers nothing
 *          falls back to the stylesheet's own two, so the guard cannot quietly go blind.
 */
export function boardGrounds(root = null, scheme = "light") {
	const read = name => {
	...
	const fallback = RELMAP_GROUND_FALLBACK[scheme] ?? RELMAP_GROUND_FALLBACK.light;
	const page = read("--relmap-page") ?? parseColour(fallback.page);
	const panel = read("--relmap-panel") ?? parseColour(fallback.panel);
	return [page, panel];
}
 ...
 * (black against a light page, white against a dark one).
 *
 ...
 */
export function resolveInkHex(key, root = null) {
	if (!root || !globalThis.getComputedStyle) return "";
	const said = globalThis.getComputedStyle(root).getPropertyValue(`--relmap-ink-${key}`);
 ...
 *
 * In one place because the board paints an ink in two element families (the stroke and the arrowheads
 * on its ends), and a line whose head is a different colour from its own body is exactly the kind of
 * wrong that only shows up on the lines that happen to have heads.
 *
 * ⚠ A COLOUR OF THE READER'S OWN IS SHOWN AT A TONE THIS READER CAN FOLLOW. The hex stored on a line was
 * checked against the ground of whoever chose it, and the reader looking at it now may be on the other
 * theme: a deep charcoal picked on the light board is no line at all on the dark one. So where
 * `grounds` are given, the colour is walked along its own hue until it clears the floor on THEM
 * (`legibleHex`). Nothing is written: every reader sees the same hue at a tone their own paper can
 * carry, and the stored colour stays exactly what was chosen.
 *
 * @param {string} ink  the stored ink: one of the eight keys, or a hex.
 * @param {object} [opts]
 * @param {Array<number[]>|null} [opts.grounds]  this reader's grounds, from `boardGrounds`.
 * @returns {{inkKey: string, inkHex: string}}  the class fragment, and the colour to write inline for a
 *          custom line ("" for one of the eight, whose colour is the stylesheet's).
 */
export function inkPaint(ink, { grounds = null } = {}) {
	const hex = normalizeHex(ink);
	if (!hex) return { inkKey: ink, inkHex: "" };
	return { inkKey: RELMAP_INK_CUSTOM, inkHex: grounds ? legibleHex(hex, grounds) : hex };
}

/** Colours already walked, by colour and ground. A board repaints often and holds few colours. */
const LEGIBLE = new Map();
const LEGIBLE_MAX = 512;

/**
 * A chosen colour at the nearest tone of its own hue that can be followed on these grounds, or the
 * colour itself where it already can. Cached, because every repaint of every board asks.
 */
export function legibleHex(hex, grounds) {
	const start = normalizeHex(hex);
	if (!start || !Array.isArray(grounds) || !grounds.length) return start;
	const key = `${start}|${grounds.map(rgb => rgb.map(v => Math.round(v)).join(",")).join("|")}`;
	if (!LEGIBLE.has(key)) {
		if (LEGIBLE.size >= LEGIBLE_MAX) LEGIBLE.clear();
		LEGIBLE.set(key, deepenInk(start, { grounds }).hex || start);
	}
	return LEGIBLE.get(key);
}
