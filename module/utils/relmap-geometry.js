...
//
// WHY IT IS ITS OWN MODULE: the arithmetic lives apart from the renderers, because a curve computed
// twice is a curve that drifts. One line is drawn three ways -- the stroke is one stretched SVG, while
// the caption and the arrowheads ride on top of it -- and all three have to agree about where the same
// line is, to the pixel, or a caption sits off its own stroke.
//
...
// is here because percentages are not a square space. A step
...
import { ROUTE_HEAD_PATH, ROUTE_HEAD_POINTS, ROUTE_HEAD_VIEWBOX } from "./arrowhead.js";

// Re-exported rather than re-authored: the triangle is drawn by the template and measured here, and
// one copy of it (utils/arrowhead.js) is what keeps the two from drifting.
 ...
 *
 * ⚠ THE STYLESHEET DRAWS THE SAME NUMBER, as the width and height of `.relmap-head`,
 ...
 * the reason utils/arrowhead.js exists: a reshaped head must not leave a second copy behind.
 *
 ...
 * tip: the two shares are the triangle's own points, for the reason utils/arrowhead.js exists, so
 ...
 * utils/arrowhead.js exists: a reshaped head must not leave a second copy of its old proportions
 ...
 *
 * ONE RING WOULD DO FOR A HANDFUL, and is exactly the look of a relationship poster. A whole
 * cast is not a handful: twenty-odd people on one ring are spaced closer than their own
...
export function freeSpot(taken = [], { aspect = RELMAP_BOARD_ASPECT, r = nodeRadiusPct(), pad = 3 } = {}) {
	...
	// board carries a crowd: the outer ring fills, the half ring fills, and everybody after that
 ...
 *
 * ⚠ THE STYLESHEET IS WHERE THIS IS ACTUALLY PAINTED (`.relmap-label-text`), and this is
