...
// `.relmap-tiebar` as a thing a press must not start a pan from; a panel outside it would
...
import {
	RELMAP_INK_CUSTOM, RELMAP_INK_PRESETS, boardGrounds, deepenInk, inkPaint, legibleHex, normalizeHex,
	resolveInkHex,
...
export class RelmapTieBar {
	...
	constructor(root, {
		...
		this.el = root?.querySelector?.(".relmap-tiebar") ?? null;
		...
		this._view = root?.querySelector?.(".relmap-view") ?? null;
	 ...
	 * `.relmap-ink--<key>` resolves to is the stylesheet's business: it is one colour under
	...
	_markInk(value) {
		...
		if (mark) {
			...
			// declares last.
			for (const key of RELMAP_INKS) mark.classList?.remove(`relmap-ink--${key}`);
			mark.classList?.remove(`relmap-ink--${RELMAP_INK_CUSTOM}`);
			mark.classList?.add(`relmap-ink--${shown}`);
	...
	_markDash(value) {
		...
		if (mark) {
			...
			for (const one of RELMAP_DASHES) mark.classList?.remove(`relmap-tiebar-rule--${one}`);
			mark.classList?.add(`relmap-tiebar-rule--${key}`);
		}
	 ...
	 * of this colour. `.relmap-ink--custom` turns the raw one into the drawn one, and under
	 ...
	 */
	_wearInk(el, hex) {
		if (hex) el?.style?.setProperty?.("--relmap-ink-raw", legibleHex(hex, this._grounds()));
	...
	/** The ground this bar's board is painted on, read off the bar, which inherits the window's tokens. */
	_grounds() {
		const scheme = this.el?.closest?.(".theme-dark") ? "dark" : "light";
		return boardGrounds(this.el ?? null, scheme);
	}
	...
	_currentHex() {
		...
		return normalizeHex(ink) || resolveInkHex(ink, this.el) || "#000000";
	}
	...
	_pickCustomInk(said) {
		...
		if (!chose) return;
		const { hex, nudged, ratio } = deepenInk(chose, { grounds: this._grounds() });
		if (!hex) return;
		if (nudged) {
