// DOM sync for the left-rail chrome the people chooser wears. Its panels switch WITHOUT
// re-rendering, so a typed filter and the picks already made survive moving between lists; that means
// the rail is synced by hand in the DOM, and everything about how it syncs lives here. A caller opts
...
export function applyGuideRail(root, {
	...
	root.querySelectorAll(tabSelector).forEach(btn => {
		...
		btn.closest(".relmap-rail-toc-item")?.classList.toggle("is-active", on);
	...
	if (iconSelector) {
		...
		if (iconEl) iconEl.className = `fas ${icon} relmap-rail-banner-icon ${iconExtraClass}`;
	}
