...
/**
 * Base class for this module's V1 windows (the map and the people chooser). It centralises the
 * two pieces of lifecycle
 ...
 */
// Through the namespace where this core has one: v13 and v14 warn about the bare global.
const ApplicationV1 = globalThis.foundry?.appv1?.api?.Application ?? globalThis.Application;

export class RelmapDialog extends ApplicationV1 {
	constructor(options = {}) {
		super(options);
	...
	reportWriteFailure(noun, err) {
		console.error(`Relationship Map | Could not apply this ${noun}.`, err);
