...
import {
	RELMAP_INK_ACROSS, RELMAP_INK_PRESETS, boardGrounds, inkPaint, legibleHex, luminance, normalizeHex,
	parseColour,
}
...
import { partyCharacters } from "../utils/party.js";
import {
	...
	setMapPageHidden, syncPartyPage,
...
// Plain literals, not built from MODULE_ID: tests/templates/partial-registration.test.js proves
...
const TEMPLATE = "modules/relationship-map-pwd/templates/dialogs/relationship-map.hbs";
const BOARD_PARTIAL = "modules/relationship-map-pwd/templates/dialogs/partials/relationship-map-board.hbs";
...
const BOARD_CONTROLS =
	...
	+ "[data-relmap-hit], [data-relmap-open], [data-relmap-action], .relmap-tiebar";
 ...
 */
const BOARD_MENUS = ".relmap-tiebar";
...
const TOOLS = Object.freeze({
	...
	// ⚠ AND THE OTHER THING THAT IS NOT AN EDIT, which is the three dials on the footer: how heavily
	// the arrowheads, the strokes and the writing are drawn FOR THIS READER. It writes to their own
	// browser and to nothing else (no document, no other window, nothing anybody else sees), and the
	// person who needs it most is once again the one who may only look. See relmap/relmap-weights.js.
	...
	// reverse whatever the reader last did on this board, wherever they did it, and they say out loud
	// what they took back. See `_stepHistory`.
	...
	// They are in this table rather than beside the strip's own click handler for the reason the table
	// exists at all: a tool gated by where it happens to be written is a tool nobody can tell was gated
	// on purpose. Switching between pages is NOT here, because it is not a tool and not an edit.
	pagenew: { needsEdit: true, run: app => app._addPage() },
	// ⚠ `needsEdit` IS NOT THE GATE THAT MATTERS HERE, and it is written all the same. Who may hide a
	// board is a narrower question than who may edit one (only a GM, because core's own sanitizer
	// refuses an ownership change from anybody else) and `_hidePage` asks it.
...
/**
 * Which notice a colour that had to be moved gets: DEEPENED on a light board, LIGHTENED on a dark one.
 * Read off the two colours rather than off the theme, so it says what actually happened.
 */
function inkMovedKey(chose, used) {
	const was = parseColour(chose);
	const now = parseColour(used);
	return was && now && luminance(now) > luminance(was) ? "RELMAP.inkLightened" : "RELMAP.inkDeepened";
}

export class RelationshipMapWindow extends RelmapDialog {
	...
	static get defaultOptions() {
		...
		return foundry.utils.mergeObject(super.defaultOptions, {
			classes: windowClasses("relmap-app"),
			template: TEMPLATE,
			width,
			height,
			resizable: true,
	 ...
	 * is a RelmapDialog only because it could not be both a dialog and a registered sheet
	...
	async getData() {
		...
		return {
			...
			pagesLabel: localize("RELMAP.pages.label"),
			...
			pageNewHint: localize("RELMAP.pages.newHint"),
			pageRenameHint: localize("RELMAP.pages.renameHint"),
			pageDeleteHint: localize("RELMAP.pages.deleteHint"),
			...
			undoNothing: localize("RELMAP.history.backNothing"),
			redoNothing: localize("RELMAP.history.forwardNothing"),
			...
			labelField: localize("RELMAP.labelField"),
			inkLabel: localize("RELMAP.inkField"),
			dirLabel: localize("RELMAP.dirField"),
			dashLabel: localize("RELMAP.dashField"),
			inks: RELMAP_INKS.map(key => ({ key, name: localize(`RELMAP.inks.${key}`) })),
			...
			inkNamed: localize("RELMAP.inkNamed"),
			...
			inkPresets: RELMAP_INK_PRESETS.map(({ key, hex }) => ({
				hex, show: legibleHex(hex, grounds), name: localize(`RELMAP.inkPresets.${key}`),
			})),
			...
			inkCustomHeading: localize("RELMAP.inkCustomHeading"),
			inkCustomNamed: localize("RELMAP.inkCustomNamed"),
			inkCustomField: localize("RELMAP.inkCustomField"),
			...
			dirs: RELMAP_DIRS.map(key => ({
				key, icon: TIE_DIR_ICONS[key], name: localize(`RELMAP.dirs.${key}`),
			})),
			// No icon: each of these DRAWS the line it means, from the stylesheet. See the template,
			// and `--relmap-dotted` / `--relmap-dashed` for the patterns they are drawn with.
			//
			...
			dashes: RELMAP_DASHES.map(key => ({
				key, name: localize(`RELMAP.dashes.${key}`),
			})),
			...
			dashNamed: localize("RELMAP.dashNamed"),
			...
			sizeLabel: localize("RELMAP.sizeField"),
			sizes: RELMAP_SIZES.map(({ key, px }) => ({
				px, name: localize(`RELMAP.sizes.${key}`),
			})),
			...
			sizeNamed: localize("RELMAP.sizeNamed"),
			...
			sizeOwnLabel: localize("RELMAP.sizeOwnLabel"),
			sizeOwnField: localize("RELMAP.sizeOwnField"),
			...
			tie: {
				label: localize("RELMAP.tie.label"),
				rub: localize("RELMAP.tie.rub"),
			},
			labelsLabel: localize("RELMAP.hideLabels"),
			labelsHint: localize("RELMAP.hideLabelsHint"),
			...
			weightGroup: localize("RELMAP.weightGroup"),
	 ...
	 * asking `RELMAP.weights.<key>.name` and about the numbers by both asking `getWeights`.
	 *
	...
	_weightDials(weights = getWeights()) {
		return RELMAP_WEIGHTS.map(key => {
			...
			const name = localize(`RELMAP.weights.${key}.name`);
			return {
				key,
				pct,
				name,
				...
				hint: localize(`RELMAP.weights.${key}.hint`),
				up: format("RELMAP.weightUp", { name }),
				down: format("RELMAP.weightDown", { name }),
				...
				reset: format("RELMAP.weightReset", { name, pct: RELMAP_WEIGHT_BASE }),
				now: format("RELMAP.weightNow", { name, pct }),
	 ...
	 * lists by kind of actor, and a row that carried only a name would land everybody under "Everyone
	...
	_personOnMap(graph, id) {
		...
		return {
			id,
			name: actor?.name || node?.name || localize("RELMAP.someone"),
			actor,
		};
	}
	...
	_boardContext({ graph, board, scales = weightScales() }) {
		...
		// How a colour of the reader's own is shown on THIS reader's ground. See `legibleHex`.
		const paint = { grounds: this._grounds() };
		...
		// sentences they are.
		const gesture = canEdit ? ` ${localize("RELMAP.removeGesture")}` : "";
		...
		const nodes = Object.entries(graph.nodes).map(([id, node]) => {
			...
			const portrait = portraitOrNone(actor?.img ?? node.img);
			...
			return {
				...
				tooltip: withHint(gone
					? format("RELMAP.deletedActor", { name })
					: actor
						? format("RELMAP.openSheet", { name })
						: name),
				linkLabel: format("RELMAP.linkFrom", { name }),
				removeLabel: format("RELMAP.removeLabel", { name }),
			};
		});
		...
		for (const shape of shapes) {
			...
			edges.push({
				...
				id, a: edge.a, b: edge.b, d: shape.d, ...inkPaint(edge.ink, paint),
			...
			for (const head of shape.heads) heads.push({ ...head, id, ...inkPaint(edge.ink, paint) });
			if (anchor) {
				...
				labels.push({
					...
					ariaLabel: canEdit
						? format("RELMAP.editLink", { label: edge.label })
						: edge.label,
				});
			}
		}
		...
		return {
			...
			linkHint: localize("RELMAP.linkHint"),
		};
	}
	...
	activateListeners(html) {
		...
		const view = root.querySelector(".relmap-view");
		const board = root.querySelector(".relmap-board");
		...
		this._tieBar = new RelmapTieBar(root, {
			...
			onNudged: (chose, used) => ui.notifications?.info?.(
				format(inkMovedKey(chose, used), { chose, used }),
			),
		...
		const strip = root.querySelector(".relmap-pages-strip");
	 ...
	 *
	 * ⚠ `changed.flags[MODULE_ID]` in BRACKETS, never a dotted path. The module id is hyphenated, so
	 * `changed.flags.relationship-map-pwd` parses as a subtraction and throws, inside a global hook,
	 * which takes down every other listener registered on it.
	 */
	_wireSync() {
		...
		on("updateJournalEntry", (doc, changed) => {
			...
			const bag = changed?.flags?.[MODULE_ID];
			if (!bag) return;
		...
		// is a RelmapDialog rather than a DocumentSheet (journal/RelationshipMapEntrySheet.js
	...
	_chrome(plan) {
		...
		return {
			...
			emptyLead: localize(
				unshared ? "RELMAP.unsharedLead" : "RELMAP.emptyLead",
			),
	...
	_paintPanel(panel, { lead, hint, action }) {
		if (!panel) return;
		const words = panel.querySelector(".relmap-empty-lead");
		if (words) words.textContent = lead ?? "";
		const said = panel.querySelector(".relmap-empty-hint");
		if (said) said.textContent = hint ?? "";
		const cta = panel.querySelector(".relmap-empty-cast");
		if (!cta) return;
		cta.hidden = !action;
		if (!action) return;
	...
	_paintChrome(plan) {
		...
		this._paintPanel(root.querySelector(".relmap-empty"), {
	...
	async _ensurePage() {
		...
		catch (err) { console.warn("Relationship Map | relationship map could not be given its first page", err); }
		...
		catch (err) { console.warn("Relationship Map | relationship map could not update the party board", err); }
	}

	/**
	 * Make or top up the board called "The Party".
	 *
	 * ⚠ THE ONE PLACE THIS WINDOW READS ANYTHING BUT THE MAP. Nobody should have to put the party on a
	 * map by hand, so the party seat themselves on a board of their own. Who the party ARE comes from
	 * `partyCharacters()` (utils/party.js), and nothing here forms a second opinion about it.
	 *
	 * ⚠ THE PRIMARY GM AND NOBODY ELSE, which is a RACE guard and the one thing here that is not
	 * obvious. This runs unasked, on open, on every client that may edit, and it mints a fresh id per
	 * person. Two people opening the map in the same minute after somebody joined the party would each
	 * seat them under a different id, and the board would hold them twice with no way to tell which to
	 * take off. A player opening the map sees whatever the GM's client has already put there.
	 */
	async _syncPartyPage() {
		if (!isPrimaryGM()) return null;
		const said = await syncPartyPage(this.entry, this._partyReaders());
		if (!said) return null;
		// SAID OUT LOUD, even though nobody asked: a tab appearing by itself, or faces arriving on a
		// board somebody is not looking at, is a change to a shared document nobody pressed a button for.
		ui.notifications?.info?.(format("RELMAP.pages.partySeeded", {
			name: said.page?.name ?? localize("RELMAP.pages.party"),
			people: said.addedPeople,
		}));
		return said;
	}

	/** The party, in the shape the seating pass wants them in. */
	_partyReaders() {
	...
	/**
	 * The two tones this reader's board is painted on, light or dark.
	 *
	 * READ OFF THE WINDOW, where the theme's tokens are declared, so a colour somebody chose is measured
	 * against the paper actually on screen. Before the window has an element (the very first render)
	 * the stylesheet's own values for the current scheme stand in. See relmap/relmap-ink.js.
	 */
	_grounds() {
		const outer = this.element?.[0] ?? null;
		const scheme = outer?.classList?.contains?.("theme-dark") ? "dark"
			: outer?.classList?.contains?.("theme-light") ? "light"
				: currentScheme();
		return boardGrounds(outer ?? this._root ?? null, scheme);
	}

	/**
	 * Called when this reader's theme or high-contrast setting changes under an open window
	 * (utils/window-theme.js). The classes are already on the root; what is left is every colour the
	 * board chose FOR the old ground, which is a render's worth of markup. A render re-fits the board,
	 * and a theme switch is rare enough to pay that.
	 */
	refreshTheme() {
	...
	_seenTool(page = this.mapPage) {
		...
		return {
			...
			pageHideHint: localize(
				dark ? "RELMAP.pages.showHint" : "RELMAP.pages.hideHint",
			),
		};
	}
	...
	_paintPages() {
		...
		const view = this._root?.querySelector(".relmap-view");
	...
	showPage(id, { said = null } = {}) {
		...
		this._armed = "";
		this._pendingSync = false;
		// "Showing The Docks" for an ordinary switch, and whatever the caller has to say instead
		// where arriving at the board is the second half of something else it did. `_addPage` is the
		// one so far, and "The Docks has been added" is news where "showing" is not.
		this._sayOnRender = said ?? format("RELMAP.pages.now", { name: page.name });
	...
	async _movePage(movedId, beforeId = null) {
		...
		try {
			...
			console.error("Relationship Map | could not move the page", err);
			ui.notifications?.warn?.(localize("RELMAP.pages.moveFailed"));
			...
			this._announce(localize("RELMAP.pages.moveFailed"));
			return false;
		}
		this._paintPages();
		...
		this._announce(format("RELMAP.pages.moved", {
	...
	async _addPage() {
		...
		if (!page) {
			ui.notifications?.warn?.(localize("RELMAP.pages.failed"));
			return;
		}
		...
		this.showPage(page.id, {
			said: format("RELMAP.pages.added", { name: page.name }),
		});
	}
	...
	async _hidePage() {
		...
		this._announce(format(
			dark ? "RELMAP.pages.hidden" : "RELMAP.pages.shown",
			{ name: page.name },
		));
	}
	...
	_getHeaderButtons() {
		...
		// Unshifted in reverse, so the maps button comes first, then the pen, then the trash, and all of
		// them in front of Close.
		if (canDeleteRelationshipMap(entry)) {
			buttons.unshift({
				label: localize("RELMAP.maps.delete"),
				class: "relmap-map-delete",
				icon: "fas fa-trash",
		...
		if (canEditRelationshipMap(entry)) {
			buttons.unshift({
				label: localize("RELMAP.maps.rename"),
				class: "relmap-map-rename",
				icon: "fas fa-pen",
				onclick: () => this._renameMap(),
			});
		}
		// THE WAY TO ANOTHER MAP, for everybody. Maps are kept out of the Journal sidebar, so this is the
		// one place a second map is reached from, and where a new one is made once the first exists.
		// Looking at a different map writes nothing, so it is not gated.
		buttons.unshift({
			label: localize("RELMAP.maps.switch"),
			class: "relmap-map-switch",
			icon: "fas fa-diagram-project",
			onclick: () => this._switchMap(),
		});
		return buttons;
	}

	/** Open another map, or make one, from the chooser. The map already open is the row it opens on. */
	async _switchMap() {
		const chosen = await chooseRelationshipMap({ current: this._entryId });
		if (!chosen || chosen.id === this._entryId) return null;
		return openRelationshipMap(chosen);
	}
	...
	async _renameMap() {
		...
		if (await renameRelationshipMap(entry, name)) {
			this._announce(format("RELMAP.maps.renamed", { name: relationshipMapName(name) }));
		}
	}
	...
	async _removeMap() {
		...
		if (await deleteRelationshipMap(entry)) {
			ui.notifications?.info(format("RELMAP.maps.deleted", { name }));
		}
	}
	...
	async _renamePage() {
		...
		if (await renameMapPage(page, name)) {
			this._announce(format("RELMAP.pages.renamed", { name: mapPageName(name) }));
		}
	}
	...
	async _removePage() {
		...
		// colour typed here.
		const ok = await this._confirm({
			title: localize("RELMAP.pages.deleteTitle"),
			body: format(
				people ? "RELMAP.pages.deleteBodyPeople" : "RELMAP.pages.deleteBody",
		...
		this._sayOnRender = format("RELMAP.pages.deleted", { name: page.name });
	...
	_toggleEmpty(empty) {
		const panel = this._root?.querySelector(".relmap-empty");
	...
	async _write(patch, { announce = "", label = "", coalesce = "", remember = true } = {}) {
		...
		try {
			...
			this.reportWriteFailure(localize("RELMAP.noun"), err);
			return false;
		}
		if (change) {
			historyFor(doc).record({
				...change,
				label: label || localize("RELMAP.history.change"),
				coalesce,
			});
			this._paintHistory();
		}
	...
	async _stepHistoryNow(way) {
		...
		if (!entry) {
			ui.notifications?.info?.(localize(`RELMAP.history.${way}Nothing`));
			return false;
		}
		...
		if (!patch) {
			...
			commit();
			ui.notifications?.info?.(localize("RELMAP.history.gone"));
			this._paintHistory();
			return false;
		}
		const said = format(`RELMAP.history.${way}Done`, { what: entry.label });
	...
	_announce(message) {
		const live = this._root?.querySelector(".relmap-live");
	...
	async _openPerson(id) {
		...
		if (!node) return;
		if (!node.uuid) {
			ui.notifications?.info?.(format("RELMAP.noSheet", { name: node.name }));
			return;
		}
		...
		return openLinkedActorSheet({ actorUuid: node.uuid }, "RELMAP.gone");
	}
	...
	async _linkFrom(id) {
		...
		if (!others.length) {
			ui.notifications?.info?.(localize("RELMAP.nobodyToLink"));
			return;
		}
	...
	_tieAt(id) {
		...
		return {
			...
			// four-valued enum.
			said: {
				none: localize("RELMAP.dirs.none"),
				both: localize("RELMAP.dirs.both"),
				"a-b": format("RELMAP.dirToward", { name: names.b }),
				"b-a": format("RELMAP.dirToward", { name: names.a }),
			},
			at,
	...
	async _createLink(a, b) {
		...
		const written = await this._write(addEdgePatch(id, { a, b, ...pen }), {
			announce: format("RELMAP.linked", {
				a: graph.nodes[a].name, b: graph.nodes[b].name,
			}),
			label: localize("RELMAP.history.linked"),
		});
	...
	async _rubOutLink(id) {
		...
		await this._write(dropEdgePatch(id), {
			announce: localize("RELMAP.unlinked"),
			label: localize("RELMAP.history.unlinked"),
		});
	}
	...
	async _removePerson(id) {
		...
		const ok = await this._confirm({
			title: localize("RELMAP.removeTitle"),
			body: links
				? format("RELMAP.removeBodyLinks", { name: node.name, count: links })
	...
	async _addPerson() {
		...
		if (!actors.length) {
			ui.notifications?.info?.(localize("RELMAP.everyoneAdded"));
			return;
		}
		// THE ACTOR RIDES ALONG on each row, and not only their name: it is what sorts the chooser into
		// its lists (the party, then each kind of actor the system has) and what puts each person's face
		// beside their name. See utils/people-groups.js.
	 ...
	 *
	 * `seatArrivals` is the same seater the party board fills itself with, which is
	...
	async _addNodesFor(graph, actors) {
		...
		return this._write(patch, {
			announce: format("RELMAP.addedCount", { count }),
			label: format("RELMAP.history.addedCount", { count }),
		});
	}
	...
	async _addNodeFor(actor, spot) {
		...
		return this._write(addNodePatch(id, {
			...
			announce: format("RELMAP.added", { name: actor.name }),
			label: format("RELMAP.history.added", { name: actor.name }),
		});
	}
	 ...
	 *
	 * WHAT IT IS FOR IS A BOARD NOBODY CAN READ. A cast of forty carries a hundred lines, every
	...
	_toggleLabels(box) {
		...
		this._announce(localize(`RELMAP.labelsNow.${this._hideLabels ? "on" : "off"}`));
	 ...
	 *
	 * Three of these stand on the footer beside the captions box (`.relmap-heft`): the
	...
	async _stepWeight(button) {
		...
		this._announce(format("RELMAP.weightNow", {
			name: localize(`RELMAP.weights.${key}.name`),
			pct: next[key],
		}));
	...
	_paintWeights(weights = getWeights()) {
		...
		for (const key of RELMAP_WEIGHTS) {
			...
			if (step) {
				...
				step.setAttribute("aria-label", format("RELMAP.weightNow", {
					name: localize(`RELMAP.weights.${key}.name`), pct,
				}));
			}
	...
	_boardEl() {
		if (this._boardRoot !== this._root) {
			...
			this._board = this._root?.querySelector?.(".relmap-board") ?? null;
		}
		return this._board;
	}
	...
	_paintLitCaptions(root) {
		const lift = root.querySelector?.(".relmap-labels-lit");
		if (!lift) return;
		...
		for (const caption of root.querySelectorAll?.(".relmap-labels .relmap-label") ?? []) {
	...
	_wireDrop(view) {
		...
		view.addEventListener("drop", async ev => {
			...
			if (Object.values(graph.nodes).some(node => node.uuid === actor.uuid)) {
				ui.notifications?.info?.(format("RELMAP.alreadyHere", { name: actor.name }));
				return;
			}
	 ...
	 *
	 * A restored board is asked for this the instant it is opened (utils/window-restore.js, or the
	 * map's own sheet), which is before its first render has finished — and `Application#minimize`
	...
	async _render(force, options) {
		...
		// the session repaints a board that is not on screen, once for every window shut this way.
		if (this._closed) {
			this._teardown();
...
function mintCaption(board, id, shape, index) {
	const layer = board?.querySelector?.(".relmap-labels");
	const doc = layer?.ownerDocument;
	if (!doc?.createElementNS) return null;
	const group = doc.createElementNS(SVG_NS, "g");
	group.setAttribute("class", "relmap-label");
	...
	words.setAttribute("class", "relmap-label-text");
 ...
 *
 * `where` is the geometry a restored window comes back at, passed in by
 * utils/window-restore.js. Nothing else in this module passes it: a map opened from the sidebar, a
 * content link or the API takes the opening size defaultOptions works out. Only finite numbers are honoured, so a
...
export function openRelationshipMap(entry, where = {}) {
	...
	// WHICH BOARD TO COME UP ON, where the caller knows: a window restored across a reload
	// (utils/window-restore.js saved the page it was left on), and a second call while the window is
	// already open. Everything else says nothing and gets the first page.
	const pageId = typeof where?.pageId === "string" ? where.pageId : null;
	// ONE FIXED TITLE, THE SAME OVER EVERY MAP AND EVERY BOARD. What this window IS does not change
	// with the map it holds or with whichever of its boards is up, so the title says what it is and the
	// strip an inch below it says which board is showing.
	const options = RelmapDialog.perDocumentOptions(MODULE_ID, entry.id, {
		title: localize("RELMAP.windowTitle"),
		...geometry,
		...(pageId ? { pageId } : {}),
	});
